...
import com.cafe.democom.cafe.dao.CategoryDao;
        ...
        try {
           if(jwtFilter.isAdmin()) {
               if(validateCategoryMap(requestMap, false)) {
                  ...
                  return CafeUtils.getResponseEntity("Category Agregada Correctamente", HttpStatus.OK);
               }
           } else {
    ...
    public ResponseEntity<List<Category>> getAllCategories(String filterValue) {
        try {
            if(!Strings.isNullOrEmpty(filterValue) && filterValue.equalsIgnoreCase("true")) {
                log.info("inside if"); //si queremos ver logs cuando el value es true
                //one or more product in that category
